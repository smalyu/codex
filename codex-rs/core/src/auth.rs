...
impl AuthStorage {
    ...
    fn load_from_file(&self) -> std::io::Result<Option<AuthDotJson>> {
        ...
        match File::open(&auth_file) {
            Ok(_) => try_read_auth_json(&auth_file).map(Some),
...
impl CodexAuth {
    ...
    /// Loads the available auth information using the default (auto) credential store mode.
    pub fn from_codex_home(codex_home: &Path) -> std::io::Result<Option<CodexAuth>> {
        Self::from_codex_home_with_store_mode(codex_home, AuthCredentialsStoreMode::Auto)
    }

    /// Loads the available auth information using an explicit credential store mode.
    pub fn from_codex_home_with_store_mode(
        codex_home: &Path,
...
pub fn logout(codex_home: &Path) -> std::io::Result<bool> {
    logout_with_store_mode(codex_home, AuthCredentialsStoreMode::Auto)
}
...
/// Writes an `auth.json` that contains only the API key using the default credential store mode.
pub fn login_with_api_key(codex_home: &Path, api_key: &str) -> std::io::Result<()> {
    login_with_api_key_with_store_mode(codex_home, api_key, AuthCredentialsStoreMode::Auto)
}
...
pub fn save_auth_with_store_mode(
    codex_home: &Path,
    auth: &AuthDotJson,
    store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<()> {
...
mod tests {
    ...
    fn login_with_api_key_overwrites_existing_auth_json() {
        ...
        super::login_with_api_key_with_store_mode(
            dir.path(),
            "sk-new",
            AuthCredentialsStoreMode::File,
        )
    ...
    fn logout_removes_auth_file() -> Result<(), std::io::Error> {
        ...
        let removed = logout_with_store_mode(dir.path(), AuthCredentialsStoreMode::File)?;
        assert!(removed);
        assert!(!dir.path().join("auth.json").exists());
        Ok(())
    }
...
impl AuthManager {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout_with_store_mode(&self.codex_home, self.store_mode)?;
